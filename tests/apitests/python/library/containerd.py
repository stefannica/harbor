...
def ctr_images_pull(username, password, oci):
    command = ["containerd-ctr", "-a", "/run/containerd/docker-containerd.sock", "images", "pull", "-u", username+":"+password, oci]
    print("Command: ", command)
    ret = base.run_command(command)
    print("Command return: ", ret)

def ctr_images_list(oci_ref = None):
    command = ["containerd-ctr", "-a", "/run/containerd/docker-containerd.sock", "images", "list", "--q"]
    print("Command: ", command)
    ret = base.run_command(command)
    print("Command return: ", ret)
